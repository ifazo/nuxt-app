                    ...
                    <div v-for="(item, itemIdx) in category.featured" :key="itemIdx"
                      ...
                      <div class="flex flex-col justify-end">
                        <div class="p-4 bg-white bg-opacity-60 text-base sm:text-sm">
                          <NuxtLink :to="item.href" class="font-medium text-gray-900">
                    ...
                    <div v-for="section in column" :key="section.name">
                      ...
                      <ul role="list" :aria-labelledby="`${category.id}-${section.id}-heading-mobile`"
                        ...
                        <li v-for="item in section.items" :key="item.name" class="flow-root">
                          <NuxtLink :to="item.href" class="-m-2 p-2 block text-gray-500">
                            {{ item.name }}
                          </NuxtLink>
                        </li>
                      </ul>
              ...
              <div class="h-full flex space-x-8">
                <Popover v-for="category in navigation.categories" :key="category.name" class="flex" v-slot="{ open }">
                  <div class="relative flex">
                    <PopoverButton
                      :class="[open ? 'text-indigo-600' : 'text-gray-700 hover:text-gray-800', 'relative z-10 flex items-center justify-center transition-colors ease-out duration-200 text-sm font-medium']">
                      {{ category.name }}
                      <span
                        :class="[open ? 'bg-indigo-600' : '', 'absolute bottom-0 inset-x-0 h-0.5 transition-colors ease-out duration-200 sm:mt-5 sm:transform sm:translate-y-px']"
                        aria-hidden="true" />
                    </PopoverButton>
                  </div>
                        ...
                        <div class="max-w-7xl mx-auto px-8">
                          <div class="grid grid-cols-2 gap-y-10 gap-x-8 py-16">
                            <div class="grid grid-rows-1 grid-cols-2 gap-8 text-sm">
                              <div v-for="(item, itemIdx) in category.featured" :key="item.name"
                                :class="[itemIdx === 0 ? 'col-span-2 aspect-w-2' : '', 'group relative aspect-w-1 aspect-h-1 rounded-md bg-gray-100 overflow-hidden']">
                                <img :src="item.imageSrc" :alt="item.imageAlt"
                                  class="object-center object-cover group-hover:opacity-75" />
                                <div class="flex flex-col justify-end">
                                ...
                                <div v-for="section in column" :key="section.name">
                                  ...
                                  <ul role="list" :aria-labelledby="`${category.id}-${section.id}-heading`"
                                    ...
                                    <li v-for="item in section.items" :key="item.name" class="flex">
                                      <NuxtLink :to="item.href" class="hover:text-gray-800">
                                        {{ item.name }}
                                      </NuxtLink>
                                    </li>
                                  </ul>
